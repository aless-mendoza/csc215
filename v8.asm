; ADD 3 DIGIT LONG NUMBERS - FIXED VERSION
...
MAIN:   CALL    TWOCR           ; DOUBLE SPACE
        ...
        ; GET TWO LINES - FIXED VERSION
        ; LINE ONE
        LXI     H,BUFFER
        CALL    READLINE        
        SHLD    NUM1STR            ; SAVE FIRST NUMBER
        XCHG                       ; FIX: Save pointer to next line
        
        ; LINE TWO
        XCHG                       ; FIX: Restore pointer for next line
        CALL    READLINE
...
; FIXED READLINE - Returns start in HL, next position in DE
READLINE:
        MOV     D,H             ; Save start position
        MOV     E,L
...
RLCR:
        ...
        INX     H               ; Move past terminator
        
RLEND:
        ; HL now points to next line
        ; DE points to start of current line
        ; Return start in HL, save next position for caller
        PUSH    H               ; Save next position
        MOV     H,D             ; Return start in HL
        MOV     L,E
        XTHL                    ; HL = start, stack = next
        POP     D               ; DE = next position
        ; Caller should save DE if they need it
        RET
...
BUILDOUTPUTFILE:
        ...
        ; Pad to next 128-byte boundary
        LXI     H,OUTBUF
        DAD     B               ; HL points to end of data
        
PADLOOP:
        MOV     A,L
        ANI     7FH             ; Check if at 128-byte boundary
        JZ      PADDONE
